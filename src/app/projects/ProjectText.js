...
const ProjectText = () => {
    ...
    return (
        ...
        <div className="col-start-1 col-span-1 p-6 transition-all hidden sm:block" >
            <h1 className={`text-4xl ml-3 md:mt-12 lg:mt-12 transition-all ${(borderInView)? "" : "md:fixed top-10 "} ${isVis? "" : "hidden"} font-bold tracking-tight text-gray-900 sm:text-6xl md:max-w-xs xl:max-w-none md:pr-12 pr-0  sm:max-w-none dark:text-gray-200`}>{projectInfo[scrollY].name}</h1>
            ...
            {projectInfo.splice(1).map(({ name, link }, index) => (  //remove first spalsh page, then add links
                <li key={index} className={`${(scrollY==(index+1))? "shadow-xl dark:ring-gray-300" : ""} rounded-full transition-all ml-1 mr-1 mt-3 mb-1 px-3 py-1 text-sm leading-6 text-gray-600 ring-1 ring-gray-900/10 hover:ring-gray-900/20 dark:ring-gray-500`}>
